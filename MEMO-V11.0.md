# MEMO Kaironaute V11.0 — Sprints O → X + Y0-Y5 + Z1-Z2 + Phase 3
...
> Dernière mise à jour : Phase 3 — tests 79/79, refactoring, code-split, Web Worker, CI, ErrorBoundary (23/03/2026)
...
| tsx@4.21.0 | `npx tsx --no-cache` — mis à jour pour compatibilité Vite 6 + Netlify |
...
## Phase 3 — Qualité, Performance, Infrastructure (23/03/2026)

> 79 tests verts · build 674 KB · Web Worker · CI GitHub Actions · ErrorBoundary
> Déployé sur Netlify + GitHub : `bc6dde5`

### P3.1 ✅ — Tests unitaires scoring engine (39 tests)

**Fichier :** `tests/test-scoring-engine.mts`

Export `@internal` de 3 fonctions pures pour testing :
- `scoreFromGroups()` — alpha-blend LIVE↔FUTURE
- `calcC4()` — concordance inter-groupes
- `calcMainScore()` — orchestrateur final (tanh unifiée + Shapley)

Tests couverts :
- Bornes [0, 100], monotonie, entiers
- C4 : deadzone 0.12, coverage 3/4 vs 2/4, cap ±0.35
- Terrain : modulation, fade-out zone haute
- Alpha-blend : LIVE pure (α=0), FUTURE pure (α=1), transition monotone
- P95 normalisation : caps par groupe (lune/9, bazi/11, ephem/7, indiv/6)
- Shapley : phi_sum + baseline ≈ score (±3)
- Stress 10K random : [0,100], entiers, NaN-free

### P3.2 ✅ — Tests simulation/intégration (17 tests)

**Fichier :** `tests/test-simulation.mts`

- calcConvergence end-to-end : score, level, breakdown, dayType, Shapley, CI
- calcDayPreview 30 jours sans NaN
- Monte Carlo 365 jours (2026) : min=32, max=93, avg=70.0, σ=13.4, 63 Cosmiques
- Observabilité 4 groupes dans breakdown
- Alpha-blend temporal smoothness
- Stress 10 profils × 7 jours = 70 calculs

### P3.3 ✅ — Refactoring calcDailyModules (975 → 4 sous-fonctions)

**Fichier :** `convergence-daily.ts`

Extraction de 4 group helpers purs (pas de mutation partagée) :

```typescript
interface GroupOutput { breakdowns: SystemBreakdown[]; signals: string[]; alerts: string[]; }
interface BaziGroupOutput extends GroupOutput { baziFamilyTotal: number; ... }
interface LuneGroupOutput extends GroupOutput { luneGroupCapped: number; ... }
interface EphemGroupOutput extends GroupOutput { ephemGroupCapped: number; ... }
interface IndivGroupOutput extends GroupOutput { indivGroupCapped: number; }
```

Orchestrateur `calcDailyModules` réduit à ~230 lignes :
```typescript
...
Cross-deps explicites : `lune.natalMoonSidForNak → _calcIndivGroup`, `lune.panchangaResult → _calcIndivGroup`

### P3.4 ✅ — Code-split React.lazy (−38% bundle initial)

**Fichier :** `App.tsx`

8 onglets chargés en lazy via `React.lazy()` + `<Suspense>` :
```typescript
...
Bundle initial : 1693 KB → 1053 KB (−38%), puis 674 KB après Firebase split

### P3.5 ✅ — Firebase chunk isolé
...
Résultat : 674 KB app + 378 KB firebase (chargé uniquement quand nécessaire)

### P3.6 ✅ — Web Worker forecast 36 mois

**Fichiers :** `src/workers/forecast.worker.ts`, `src/hooks/useForecastWorker.ts`, `ForecastTab.tsx`

Le calcul lourd (1080 appels `calcDayPreview`) tourne dans un thread séparé :
```typescript
const { forecast, loading, durationMs, workerUsed } = useForecastWorker(
  bd, data.num, data.cz, data.astro, 0,
);
```

Fallback automatique sur main thread si Worker indisponible. Vite bundle les dépendances dans le worker automatiquement (~151 KB isolé).

### P3.7 ✅ — ErrorBoundary global

**Fichiers :** `src/components/ErrorBoundary.tsx`, `src/main.tsx`

Capture les crashes React, affiche un fallback élégant ("Perturbation temporelle") avec boutons Réessayer/Recharger. Stack trace visible en mode dev uniquement. Wraps `<App />` dans `main.tsx`.

### P3.8 ✅ — GitHub Actions CI

**Fichier :** `.github/workflows/ci.yml`

Pipeline sur chaque push/PR :
1. `npm ci` (install)
2. `npm run typecheck` (zéro erreur TS)
3. `npm test` (79 tests : 23 structurels + 39 scoring + 17 simulation)
4. `npm run build` (prod)
5. Bundle size check (fail si main > 800 KB)

### P3.9 ✅ — Coverage report

**Script :** `npm run test:coverage`

Utilise c8 sur le scoring engine → rapport text + HTML dans `coverage/`.

### P3.10 ✅ — Responsive mobile

**Fichiers :** `src/styles/global.css`, `IChingTab.tsx`, `ProfileTab.tsx`

```css
@media (max-width: 480px) {
  .flex-wrap-mobile { flex-wrap: wrap !important; }
  .grid-profile-4 { grid-template-columns: 1fr 1fr !important; }
}
```

### P3.11 ✅ — npm test unifié + tests organisés

**Dossier :** `tests/`

```
tests/test-phase2.mjs          — 23 tests structurels
tests/test-scoring-engine.mts  — 39 tests scoring engine
tests/test-simulation.mts      — 17 tests simulation
tests/snapshots/                — snapshots régression
```

Script unifié : `npm test` lance les 3 suites séquentiellement.

### P3.12 — Fix déploiement Netlify

`tsx@3.14.0` → `tsx@4.21.0` (peer dep conflict avec Vite 6 sur Netlify).

---

## Bundle Size Report (post Phase 3)

| Chunk | Taille | Gzip |
|-------|--------|------|
| index (app core) | 674 KB | 232 KB |
| firebase | 378 KB | 95 KB |
| forecast.worker | 151 KB | — |
| BondTab | 136 KB | 45 KB |
| ProfileTab | 120 KB | 34 KB |
| AstroTab | 104 KB | 31 KB |
| ConvergenceTab | 93 KB | 26 KB |
| IChingTab | 57 KB | 12 KB |
| CalendarTab | 49 KB | 13 KB |
| TemporalTab | 31 KB | 8 KB |
| LectureTab | 28 KB | 8 KB |
...
|----------|----------|--------|
| 🔴 P1 | Firebase Auth (Google + email) + Firestore rules sécurisées | En attente |
| 🔴 P1 | Stripe Checkout (abonnement ou crédits) | En attente |
| 🟡 P2 | RGPD : CGU, privacy policy, consentement, export/suppression données | En attente |
| 🟡 P2 | Landing page + tunnel conversion free→premium | En attente |
| 🟢 P3 | Analytics (Plausible/Posthog) | En attente |
