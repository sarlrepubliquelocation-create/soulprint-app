#!/usr/bin/env npx tsx
/**
 * Diagnostic Script for Jérôme × Laurent Compatibility
...
const MODE = 'famille'; // frère de Jérôme
...
async function diagnose(): Promise<void> {
  ...
  console.log('║  Jérôme (1977-09-23) × Laurent (1981-08-17)                       ║');
  console.log('╚════════════════════════════════════════════════════════════════════╝');
  ...
  printSubsection('Laurent (1981-08-17)');
  const lNum = calcNumerology(LAURENT.firstName, '', '', LAURENT.birthDate, TODAY);
  ...
  printSubsection('Laurent Day Master');
  const lDayMaster = calcDayMaster(new Date(LAURENT.birthDate + 'T12:00:00Z'));
  ...
  const baziCompat = calcBaZiCompat(
    ...
    new Date(LAURENT.birthDate + 'T12:00:00Z')
  );
  ...
  const lPeachBlossom = getPeachBlossom(new Date(LAURENT.birthDate + 'T12:00:00Z'), new Date(TODAY + 'T12:00:00Z'));
  ...
  printKeyValue('Laurent PB Active', lPeachBlossom?.isActive ? 'YES' : 'NO');
  if (lPeachBlossom) {
    printKeyValue('Laurent PB Details', `${lPeachBlossom.animalName} - ${lPeachBlossom.dynamique}`);
  }
  ...
  printSubsection('Laurent Natal Hexagram');
  const lNatalHex = calcNatalIChing(LAURENT.birthDate);
  ...
  const lDailyHex = calcIChing(LAURENT.birthDate, TODAY);
  printKeyValue('Jérôme Daily Hex', `#${jDailyHex.hexNum} - ${jDailyHex.name}`);
  printKeyValue('Laurent Daily Hex', `#${lDailyHex.hexNum} - ${lDailyHex.name}`);
  ...
  console.log(`    • Laurent CdV: ${lNum.lp.v} | Expression: ${lNum.expr.v} | Soul: ${lNum.soul.v}`);
  console.log(`    • Jérôme BaZi: ${jDayMaster.stem.element} ${jDayMaster.stem.pinyin} / ${jDayMaster.branch.animal}`);
  console.log(`    • Laurent BaZi: ${lDayMaster.stem.element} ${lDayMaster.stem.pinyin} / ${lDayMaster.branch.animal}`);
  console.log(`    • Jérôme Natal Hex: #${jNatalHex.hexNum} - ${jNatalHex.name}`);
  console.log(`    • Laurent Natal Hex: #${lNatalHex.hexNum} - ${lNatalHex.name}`);
