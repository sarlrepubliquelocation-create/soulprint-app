...
export default function ProfileTab({ data, bd, bt, gender = 'M', fn = '' }: { data: SoulData; bd: string; bt?: string; gender?: 'M' | 'F'; fn?: string }) {
  ...
  async function generateAnnualCard() {
    ...
    try {
      ...
      ctx.font = '11px Arial,sans-serif'; ctx.fillStyle = GOLD;
      ctx.fillText(`VOS FENÊTRES COSMIQUES  ${year}`, W / 2, 358);
  ...
  return (
    <div>
      ...
      {/* ═══ ADN NUMÉROLOGIQUE — INCLUSION — V9 Sprint 5 ═══ */}
      <Sec icon="🧬" title="ADN Numérologique">
        <Cd>
          {!incl ? (
            ...
            <div style={{ textAlign: 'center', padding: '24px 0' }}>
              ...
              <div style={{ fontSize: 12, color: P.textDim, lineHeight: 1.6 }}>
                Renseignez votre prénom et nom de naissance dans votre profil<br/>
                pour révéler vos manques karmiques et plans de conscience.
              </div>
            </div>
          ) : (
            <>
              ...
              <div style={{ fontSize: 11, color: P.textDim, marginBottom: 12, lineHeight: 1.6, fontStyle: 'italic' }}>
                Portrait statique basé sur le nom de naissance · {incl.N} lettres analysées
              </div>
                ...
                {[1,2,3,4,5,6,7,8,9].map(d => {
                  ...
                  return (
                    <div key={d} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      {/* Domaine */}
                      <div style={{ fontSize: 9, color: P.textDim, width: 68, textAlign: 'right', whiteSpace: 'nowrap', overflow: 'hidden' }}>
                    ...
                    <div key={d} style={{
                      ...
                      <div>
                        <div style={{ fontSize: 11, fontWeight: 700, color: '#ef4444' }}>
                          Manque de {d} — {INCLUSION_DOMAIN_MAP[d].lesson}
                        </div>
                        <div style={{ fontSize: 10, color: P.textDim, marginTop: 2 }}>
                          Axe {INCLUSION_DOMAIN_MAP[d].domain} · à cultiver dans cette vie
                        </div>
                      </div>
                    </div>
                  ))}
      ...
      <Sec icon="🃏" title="Carte Natale — Tarot">
        <Cd>
          <div style={{ fontSize: 12, color: P.textDim, lineHeight: 1.7, marginBottom: 14, fontStyle: 'italic' }}>
            Dans Kaironaute, le Tarot est un miroir archétypal au sens jungien — pas un oracle. Votre Carte Natale est calculée depuis votre date de naissance et révèle l'énergie archétypale qui vous accompagne tout au long de votre vie.
          </div>

          <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start', marginBottom: 16 }}>
            {/* Visuel de la carte */}
            <div style={{ width: 64, height: 104, background: `${P.gold}12`, borderRadius: 10, border: `2px solid ${P.gold}44`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flexShrink: 0, gap: 4 }}>
              <span style={{ fontSize: 32 }}>🃏</span>
              <span style={{ fontSize: 10, color: P.gold, fontWeight: 700 }}>Arcane {birthCard.num}</span>
            </div>
      ...
      <Sec icon="✨" title="Carte Annuelle">
        <Cd>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.85)', fontWeight: 600 }}>{fn || 'Votre Nom'}</div>
            <div style={{
